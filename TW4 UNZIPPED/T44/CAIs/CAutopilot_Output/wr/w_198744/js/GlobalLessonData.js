...
(function() {
	...
	window["GlobalLessonData"] = function GlobalLessonData() {				
		$("#navMenuButton").prop("disabled", false);
		parent.document.addEventListener("keydown", handleKeyDown);
	...
	GlobalLessonData.prototype.loadLessonData = function(scope) {
		...
		this._jqXhr = $.ajax({
			...
			error: function(jqXhr, textStatus, errorThrown) {
				alert("Unable to load lesson data.");
				this._jqXhr = null;
			}.bind(this),
			success: function(xml) {
				...
				// dynamically apply the 'slideNum' attribute to each <page> node for the entire lesson
				var topics = _xmlData.getElementsByTagName("topic");
				for(var i=0; i<topics.length; i++) {
					addSlideNumAttr(topics[i]);
				}
...
// 
	...
	function handleKeyDown(e) {
		...
		// show the Unlock/Lock password dialog, 
		// for now using SHIFT-F8 since F10 is used by the Flash Player, and F5, F6, F7 are used by Internet Explorer
		if ((e.shiftKey == true) && (e.key == "F8")) {
			...
			if (_lessonType.toLowerCase() == "cai"){
				...
				$("#passwordDialog").append('<input id="passwordInput" type="password" placeholder="Enter Password"/>');
				$("#passwordDialog").append('<input id="passwordSubmit" type="button" value="Submit">');
				$("#passwordDialog").append('<input id="passwordClose" type="button" value="Cancel">');
				$("#passwordInput").focus();
				$("#passwordSubmit").click(function(){checkLockStepPassword()});
				$("#passwordInput").keypress(handlePasswordKeyUp);
	...
	function handlePasswordKeyUp(e) {
		...
		//maybeTrace("[cnatra.GlobalLessonData.as] handlePasswordKeyDown; e.keyCode: " + e.keyCode);
		
		// check for ENTER key
		if (e.keyCode == 13) {
			// do password check logic
			checkLockStepPassword();
		}
		
		// check for ESC key
		if (e.keyCode == 27) {
			// hide dialog box (just like clicking Cancel)
			$("#passwordDialog").remove();	
		}
	
	}

	function checkLockStepPassword() {
	
		var checkPassword = "";
	
		// if we are currently in LockStep mode (student) then we are looking for 'CNATRAFTSC' to unlock
		// else if we are currently unlocked then we are looking for 'student' to put the lesson back into lock-step
		if (_lessonIsLockStep == true) {
			checkPassword = "CNATRAFTSC";
		} else {
			checkPassword = "student";
		}
		
		// check the password against what was entered, for now it is case sensitive
		if ($("#passwordInput").val() == checkPassword) {
			
			// flip our lockstep flag
			_lessonIsLockStep = !_lessonIsLockStep;

			if(!_lessonIsLockStep){
				_xmlData.setAttribute("lessonIsLockstep", false);
			}else{
				if(_xmlData.hasAttribute("lessonIsLockstep")){
					_xmlData.removeAttribute("lessonIsLockstep");
				}
			}					
			
			var xmlText = new XMLSerializer().serializeToString(_xmlData);
			sessionStorage.setItem("lessonXML", xmlText);
			
			if (_capVarsHandle.cpInfoCurrentSlide == 1) {
				if(window.parent["mainMenu"] != null){
					window.parent["mainMenu"].refresh();
				}
			}
